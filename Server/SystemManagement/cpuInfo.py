...
def main(p=True):
    cpu_times_data = get_cpu_times(None, p)
    cpu_usage_data = get_cpu_usage(None, p)
    cpu_freq_data = get_cpu_freq(None, p)
    data = {
...
def get_cpu_times(n=None, p=True):
...
def get_cpu_usage(n=None, p=True):
    templ = "%-2s %-6s %6s"
    cpu_usage_data = None
...
def get_cpu_freq(n=None, p=True):
